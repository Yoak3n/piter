...
use crate::pi::{ensure_pi_binary, locked_pi_version};
...
pub fn configure(builder: Builder<tauri::Wry>) -> Builder<tauri::Wry> {
    ...
    builder.setup(|app| {
        let pi_exe = ensure_pi_binary(app.handle())?;
        ...
        let (gw_state, _port) =
            pi_server::gateway::GatewayState::start_gateway(pi_exe, pi_version, dist_path, dev_port, None)
                .map_err(|e| format!("Failed to start gateway: {}", e))?;

        let ws_url = gw_state.ws_url();
        let http_url = gw_state.http_url();

        app.manage(gw_state);

        std::env::set_var("PI_BROKER_URL", &ws_url);
        std::env::set_var("PI_HTTP_URL", &http_url);
        log::info!("Pi gateway: WS={} HTTP={}", ws_url, http_url);
        ...
        if dev_port.is_none() {
            let _ =
                WM::global().show_window(WindowType::Main, Some(&format!("{}chat", http_url)));
        }
...
pub fn app_event_handle(app_handle: &AppHandle, event: RunEvent) {
    match event {
        ...
        tauri::RunEvent::ExitRequested { api, code, .. } if code.is_none() => {
            if let Some(gw) = app_handle.try_state::<Arc<crate::pi::GatewayState>>() {
