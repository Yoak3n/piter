...
.tool-name { color:var(--accent); font-family:var(--font-mono); font-size:11px; }
.tool-args-preview { color:var(--text-tertiary); font-family:var(--font-mono); font-size:11px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; max-width:300px; }
.tool-status {
  font-size:10px; padding:2px 7px; border-radius:var(--radius-pill);
  text-transform:uppercase; letter-spacing:0.04em; flex-shrink:0;
  display:flex; align-items:center; gap:4px;
}
.tool-status.pending { background:var(--bg-panel); color:var(--text-tertiary); border:1px solid var(--border); }
.tool-status.pending::before { content:"○"; font-size:8px; }
.tool-status.streaming { background:var(--accent-soft); color:var(--accent-strong); border:1px solid color-mix(in srgb, var(--accent) 35%, transparent); animation:pulse 1.5s infinite; }
.tool-status.streaming::before { content:"●"; font-size:7px; }
.tool-status.complete { background:var(--success-soft); color:var(--success); border:1px solid color-mix(in srgb, var(--success) 22%, transparent); }
.tool-status.complete::before { content:"✓"; font-size:9px; }
.tool-status.error { background:var(--danger-soft); color:var(--danger); border:1px solid color-mix(in srgb, var(--danger) 22%, transparent); }
.tool-status.error::before { content:"!"; font-size:9px; }
@keyframes pulse { 0%,100%{ opacity:1; } 50%{ opacity:0.7; } }
.tool-card-body { border-top:1px solid var(--border); }
.tool-args {
  background:var(--bg-active); padding:10px 12px;
